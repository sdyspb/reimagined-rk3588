# Модульная платформа для IP-камер и распределённых систем видеонаблюдения и машинного зрения 

**Назначение проекта** — создание открытого модульного решения на базе RK3588 для подключения промышленных MIPI-камер с полной поддержкой в ядре Linux, обеспечивающего гибкую аппаратную конфигурацию (SoM + интерфейсные платы) и лёгкую интеграцию в распределённые системы видеонаблюдения и машинного зрения. Рассматривается также возможность интеграции в платформу чувствительных сенсоров с целью разработки специализированных камер с функциями искусственного интеллекта.

Проект реализуется по принципу аппаратной модульности (SoM + интерфейсная плата). В основе лежит System-on-Module (SoM) на базе RK3588, который объединяет сам процессор, до 32 ГБ LPDDR4x/LPDDR5, до 256 ГБ eMMC и PMIC. В зависимости от степени интеграции, площадь модуля может варьироваться от полного размера, до половины кредитной карты. Такое решение позволяет отделить высокоскоростную, термонагруженную и сложную в разводке часть системы в отдельную составляющую. Интерфейсная плата проектируется под конкретные требования заказчика: на неё выносятся все внешние разъёмы (MIPI CSI, USB 3.0, PCIe, Ethernet, промышленные разъёмы питания и ввода-вывода). Это даёт возможность быстро адаптировать модуль к различным сценариям — от компактной встраиваемой системы, например, IP камеры с функциями AI, до многоканального видеосервера.

Ключевым преимуществом такой архитектуры является лёгкая встраиваемость в распределённые системы. Модуль может выступать как самостоятельный edge-узел, так и часть кластера, взаимодействуя через стандартные сетевые интерфейсы (GbE, PCIe) и поддерживая протоколы OPC UA, MQTT, REST API. Программная модульность подкрепляется возможностью контейнеризации (Docker) и микросервисной организации, что позволяет изолировать задачи захвата видео, AI-инференса и управления, а также масштабировать систему горизонтально.

Процессор Rockchip RK3588 поддерживает [подключение до 6 камер](docs/performance/camera_config_example.md) c MIPI CSI за счёт встроенного контроллера камер (Image Signal Processor, ISP) и возможности разделения дифференциальных пар (lane) отдельных блоков.

С точки зрения поддержки камер в ОС Linux, проект опирается на два подхода к драйверам MIPI CSI. В основной ветке (mainline) ядра Linux ведётся активная работа по добавлению драйвера rockchip-cif (Camera Interface) для RK3588. На текущий момент драйвер поддерживает режим VICAP для платформы RK3568, а полная поддержка RK3588 ожидается в будущих патчах. Это перспективное направление для проектов, стремящихся к использованию чистого upstream-ядра.

Для промышленной разработки, требующей стабильности и полной функциональности, используется BSP (Board Support Package) от Rockchip. Официальный SDK Rockchip включает драйверы rkcif и rkisp, которые полностью реализуют все аппаратные возможности: приём данных с шести MIPI CSI каналов, работу VICAP в режимах online/offline, аппаратный ISP с шумоподавлением, HDR и цветокоррекцией. Этот выбор гарантирует предсказуемую работу с готовыми Device Tree конфигурациями и полной поддержкой 6 камер, что критично для промышленных систем видеонаблюдения, машинного зрения и робототехники.
...
- Сетевые интерфейсы:
    - Два встроенных Gigabit Ethernet (GbE - 1 Гбит/с): Интеграция с IP-камерами по протоколу RTSP.
...
Энергопотребление модуля на основе RK3588 в среднем составляет 6-15 Вт, что при максимально допустимой температуре 70-85°C для промышленных устройств, определяет выбор системы охлаждения:

- Тип системы охлаждения - это некий компромисс между габаритами и защитой от внешнего влияния среды:
    - Пассивное охлаждение: массивный радиатор (с площадью поверхности 40-100 см²) для герметичных корпусов без пыли и влаги.
...
- Теплоотводящий интерфейс:
    ...
    - Теплотрубки диаметром 6-8 мм эффективны для сложных конструкций. При 6 Вт на 6 мм трубку, двух трубок должно хватить на систему с одним процессором. 
